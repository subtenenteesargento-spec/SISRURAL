...
window.openPhotoEditorForBase=async(idx)=>{
  ...
  if(!cp){
    ...
    const data={nome:b.nm||b.nome||'Propriedade',tipo:b.tp||b.tipo||'',endereco:b.end||'',telefone:b.ph||'',lat:b.lat,lng:b.lng,dirt:!!b.dirt,maps:b.gmaps||`https://www.google.com/maps/dir/?api=1&destination=${b.lat},${b.lng}&travelmode=driving`,municipio:'Casa Branca',quadrante:b.q||classQ(b.lat,b.lng),origem:'base_enriquecida',baseIndex:Number(idx),usuario:v7User?.email||'',createdAt:serverTimestamp(),updatedAt:serverTimestamp()};
...
async function findOrCreateCloudPropertyForBase(baseIndex){
  ...
  const data={nome:b.nm||b.nome||'Propriedade',tipo:b.tp||b.tipo||'',atividade:b.tp||b.tipo||'',plantioInicio:0,plantioFim:0,colheitaInicio:0,colheitaFim:0,epocaPlantio:'',epocaColheita:'',sazonalidadeConfigurada:false,endereco:b.end||'',telefone:b.ph||'',lat:b.lat,lng:b.lng,dirt:!!b.dirt,maps:b.gmaps||`https://www.google.com/maps/dir/?api=1&destination=${b.lat},${b.lng}&travelmode=driving`,municipio:'Casa Branca',quadrante:b.q||classQ(b.lat,b.lng),origem:'base_enriquecida',baseIndex:Number(baseIndex),usuario:v7User?.email||'',createdAt:serverTimestamp(),updatedAt:serverTimestamp()};
  const ref=await addDoc(collection(db,'propriedades_cadastradas'),data); cp={id:ref.id,...data};
...
function adminPropertyRows(){
  const base=(window.PROPS||[]).map((p,i)=>({key:'base_'+i,source:'base',baseIndex:i,id:'base_'+i,nome:p.nm||'',tipo:p.tp||'',endereco:p.end||'',telefone:p.ph||'',lat:+p.lat,lng:+p.lng,quadrante:p.q||classQ(+p.lat,+p.lng)}));
  const cloud=(v7CloudProps||[]).filter(p=>p.origem!=='base_enriquecida').map(p=>({key:'cloud_'+p.id,source:'cloud',id:p.id,nome:p.nome||p.nm||'',tipo:p.tipo||p.tp||'',endereco:p.endereco||p.end||'',telefone:p.telefone||p.ph||'',lat:+p.lat,lng:+p.lng,quadrante:p.quadrante||classQ(+p.lat,+p.lng)}));
  return base.concat(cloud).sort((a,b)=>String(a.nome).localeCompare(String(b.nome),'pt-BR'));
}
function searchablePropertyRows(){
  const rows=adminPropertyRows();
  const local=(window.userPts||[]).map((p,i)=>({key:'local_'+i,source:'local',localIndex:i,id:'local_'+i,nome:p.nm||p.nome||'',tipo:p.tp||p.tipo||'',endereco:p.end||p.endereco||'',telefone:p.ph||p.telefone||'',lat:+p.lat,lng:+p.lng,quadrante:p.q||classQ(+p.lat,+p.lng)}));
  return rows.concat(local);
}
function applyBaseEnrichmentOverrides(){
  (v7CloudProps||[]).filter(p=>p.origem==='base_enriquecida').forEach(e=>{
    let idx=Number.isInteger(Number(e.baseIndex)) && Number(e.baseIndex)>=0 ? Number(e.baseIndex) : -1;
    if(idx<0){
      let best=-1,bestD=999999; (window.PROPS||[]).forEach((b,i)=>{const d=distMeters(+b.lat,+b.lng,+e.lat,+e.lng);if(d<bestD){bestD=d;best=i;}}); if(bestD<=80) idx=best;
    }
    const b=(window.PROPS||[])[idx]; if(!b)return;
    if(e.nome) b.nm=e.nome; if(e.tipo||e.atividade)b.tp=e.tipo||e.atividade; if(e.endereco)b.end=e.endereco; if(e.telefone!==undefined)b.ph=e.telefone;
    try{window.refreshBasePropertyPopup&&window.refreshBasePropertyPopup(idx);}catch(_){ }
  });
}
window.openPropertySearch=()=>{ if(!v7User)return alert('Faça login para pesquisar propriedades.'); const inp=$v('propSearchInput'); if(inp)inp.value=''; $v('v7PropertySearchModal')?.classList.add('open'); renderPropertySearch(); setTimeout(()=>inp?.focus(),100); };
window.closePropertySearch=()=>{$v('v7PropertySearchModal')?.classList.remove('open');};
window.renderPropertySearch=()=>{
  const q=normTxt($v('propSearchInput')?.value||''); let arr=searchablePropertyRows();
  if(q) arr=arr.filter(p=>normTxt([p.nome,p.tipo,p.endereco,'Q'+p.quadrante,v7QLabel(p.quadrante)].join(' ')).includes(q));
  arr=arr.slice(0,80); const c=$v('propSearchCount'); if(c)c.textContent=`${arr.length} resultado(s)`;
  const el=$v('propSearchResults'); if(!el)return; el.innerHTML=arr.map(p=>`<div class="v7-card" style="display:flex;justify-content:space-between;gap:8px;align-items:center;margin:6px 0"><div><b>${String(p.nome).replace(/[<>]/g,'')}</b><div class="v7-small">${v7QLabel(p.quadrante)} · ${(p.tipo||'Atividade não informada').replace(/[<>]/g,'')}<br>${(p.endereco||'').replace(/[<>]/g,'')}</div></div><button class="btn-primary" style="width:auto;min-width:96px;margin:0" onclick="goToPropertySearchResult('${p.key}')">📍 Localizar</button></div>`).join('')||'<div class="v7-card">Nenhuma propriedade encontrada.</div>';
};
window.goToPropertySearchResult=(key)=>{ window.closePropertySearch(); if(String(key).startsWith('base_')) return window.zProp?.(Number(String(key).slice(5))); if(String(key).startsWith('cloud_')) return window.zCloudProp?.(String(key).slice(6)); if(String(key).startsWith('local_')) return window.zUserProp?.(Number(String(key).slice(6))); };
...
window.openPropertyCorrection=async(key)=>{
  ...
  if(p.source==='base'){
    const cp=await findOrCreateCloudPropertyForBase(p.baseIndex); if(!cp.baseIndex && cp.id) try{await setDoc(doc(db,'propriedades_cadastradas',cp.id),{baseIndex:p.baseIndex},{merge:true});}catch(_){ }
    p={...p,cloudId:cp.id};
  } else p.cloudId=p.id;
  $v('propCorrectionKey').value=JSON.stringify({key:p.key,source:p.source,baseIndex:p.baseIndex??null,cloudId:p.cloudId,oldName:p.nome}); $v('propCorrectionName').value=p.nome||''; $v('propCorrectionType').value=p.tipo||''; $v('propCorrectionAddress').value=p.endereco||''; $v('propCorrectionPhone').value=p.telefone||''; $v('propCorrectionMsg').textContent=''; $v('propCorrectionTitle').textContent='✏️ '+(p.nome||'Retificar propriedade'); $v('v7PropertyCorrectionModal')?.classList.add('open');
};
window.savePropertyCorrection=async()=>{
  if(!isAdminGeral())return alert('Somente Administrador Geral.'); let meta; try{meta=JSON.parse($v('propCorrectionKey').value||'{}')}catch(e){return alert('Cadastro inválido.')} const nome=$v('propCorrectionName').value.trim(); if(!nome)return alert('Informe o nome correto.');
  const data={nome,tipo:$v('propCorrectionType').value.trim(),atividade:$v('propCorrectionType').value.trim(),endereco:$v('propCorrectionAddress').value.trim(),telefone:$v('propCorrectionPhone').value.trim(),retificadoPor:v7User.email,retificadoEm:serverTimestamp(),updatedAt:serverTimestamp()};
  try{
    await setDoc(doc(db,'propriedades_cadastradas',meta.cloudId),data,{merge:true});
    const old=normTxt(meta.oldName); for(const v of (v7Visits||[])){ if(normTxt(v.propriedade)===old || String(v.propriedadeId||'')===String(meta.cloudId)){ await setDoc(doc(db,'visitas',v.id),{propriedade:nome,updatedAt:serverTimestamp()},{merge:true}); } }
...
function duplicateCandidates(dup){
  ...
  return rows.map(p=>({p,d:distMeters(+p.lat,+p.lng,+dup.lat,+dup.lng),nameSame:normTxt(p.nome)===normTxt(dup.nome)})).filter(x=>x.nameSame||x.d<=100).sort((a,b)=>(b.nameSame-a.nameSame)||(a.d-b.d));
}
window.mergeDeleteDuplicateProperty=async(id)=>{
  if(!isAdminGeral())return alert('Somente Administrador Geral.'); const dup=(v7CloudProps||[]).find(p=>String(p.id)===String(id)); if(!dup)return alert('Cadastro não localizado.'); const cand=duplicateCandidates({id:dup.id,nome:dup.nome||'',lat:+dup.lat,lng:+dup.lng});
  if(!cand.length)return alert('Não encontrei outro cadastro suficientemente próximo ou com o mesmo nome. Para segurança, o SISRURAL não apagou nada.'); const keep=cand[0].p;
  const ok=confirm(`TRATAR DUPLICIDADE\n\nExcluir: ${dup.nome||'cadastro'}\nManter: ${keep.nome}\n\nAs visitas serão vinculadas ao cadastro mantido. Confirmar?`); if(!ok)return;
  try{
    let keepId=keep.id, keepName=keep.nome, keepSource=keep.source;
    if(keep.source==='base'){ const cp=await findOrCreateCloudPropertyForBase(keep.baseIndex); keepId=String(keep.baseIndex); keepName=keep.nome; keepSource='base'; }
    const mergeData={}; ['foto1Url','foto2Url','foto1PublicId','foto2PublicId','fotosUrls','fotosPublicIds','fotosQuantidade','fotosProvider','atividade','tipo','plantioInicio','plantioFim','colheitaInicio','colheitaFim','epocaPlantio','epocaColheita','sazonalidadeConfigurada'].forEach(k=>{if(dup[k]!==undefined&&dup[k]!==''&&dup[k]!==0)mergeData[k]=dup[k];});
    if(keep.source==='cloud' && Object.keys(mergeData).length) await setDoc(doc(db,'propriedades_cadastradas',keep.id),mergeData,{merge:true});
    for(const v of (v7Visits||[])){ if(String(v.propriedadeId||'')===String(dup.id)||normTxt(v.propriedade)===normTxt(dup.nome)){ await setDoc(doc(db,'visitas',v.id),{propriedadeId:keepId,propriedade:keepName,origem:keepSource,updatedAt:serverTimestamp()},{merge:true}); } }
    await deleteDoc(doc(db,'propriedades_cadastradas',dup.id)); await auditV7('duplicidade_propriedade_tratada',`${dup.nome} excluída; mantida ${keepName}`); alert('✅ Duplicidade tratada. O histórico foi preservado no cadastro mantido.'); renderAdminPropertyManager();
  }catch(e){alert('Não foi possível tratar a duplicidade: '+(e.message||e));}
};
...
function v7AllPropsForReport(){
  ...
  const base=(window.PROPS||[]).map(p=>({
    ...
    municipio:p.municipio||'Casa Branca',
    quadrante:p.quadrante||p.q||window.v15ClassQMunicipal?.(p.lat,p.lng,p.municipio||'Casa Branca')||classQ(p.lat,p.lng),origem:'base'
  }));
  const local=(window.userPts||[]).map(p=>({
    nome:p.nm||p.nome,tipo:p.tp||p.tipo,lat:p.lat,lng:p.lng,
    municipio:p.municipio||'Casa Branca',
    quadrante:p.quadrante||p.q||window.v15ClassQMunicipal?.(p.lat,p.lng,p.municipio||'Casa Branca')||classQ(p.lat,p.lng),origem:'local'
  }));
  const cloud=(v7CloudProps||[]).map(p=>({
    ...
    lat:p.lat,lng:p.lng,
    municipio:p.municipio||'Casa Branca',
    quadrante:p.quadrante||window.v15ClassQMunicipal?.(p.lat,p.lng,p.municipio||'Casa Branca')||classQ(p.lat,p.lng),
    endereco:p.endereco||'',
    telefone:p.telefone||'',
    maps:p.maps||'',
    origem:'nuvem',
  ...
  [...base,...local,...cloud].forEach(p=>{
    if(!p.nome) return;
    const k=normTxt(p.municipio||'Casa Branca')+'|'+normTxt(p.nome)+'|'+(p.quadrante||'');
    m.set(k,{...(m.get(k)||{}),...p});
  });
  return [...m.values()];
}

function v15ReportScopeMunicipio(){
  const a=window.V15_ACTIVE_AREA||'Casa Branca';
  return a==='2cia'?'':a;
}
function v7VisitMunicipio(v){ return v.municipio || v.cidade || 'Casa Branca'; }
function v7VisitQuadrante(v){
  const mun=v7VisitMunicipio(v);
  return v.quadrante || v.q || (v.lat&&v.lng?(window.v15ClassQMunicipal?.(v.lat,v.lng,mun)||classQ(v.lat,v.lng)):'');
}
...
function v7ReportStats(scopeOverride){
  ...
  const visitKey=v=>normTxt(v._municipio||'Casa Branca')+'|'+normTxt(v._prop);
  const propKey=p=>normTxt(p.municipio||'Casa Branca')+'|'+normTxt(p.nome);
  const visitedNames=new Set(visits.map(visitKey).filter(Boolean));
  const never=props.filter(p=>!visitedNames.has(propKey(p)));
  const olderDays=(days)=>props.filter(p=>{
    const n=propKey(p); const pv=visits.filter(v=>visitKey(v)===n && v._dt).sort((a,b)=>b._dt-a._dt)[0];
    if(!pv) return false;
    return (now-pv._dt)/(1000*60*60*24)>days;
  });
  const latestByProp=new Map();
  visits.filter(v=>v._prop&&v._dt).sort((a,b)=>b._dt-a._dt).forEach(v=>{ const k=visitKey(v); if(!latestByProp.has(k)) latestByProp.set(k,v); });
...
// V13.1 — Ocorrências rurais para inteligência territorial (não substitui BOPM)
function openOccurrenceForProperty(p){
  if(!v7User) return alert('Faça login para registrar a ocorrência.');
  try{map.closePopup();}catch(e){}
  const now=new Date(), pad=n=>String(n).padStart(2,'0');
  $v('v7OccurrenceKey').value=p.key; $v('v7OccurrenceProp').value=p.nome;
  $v('v7OccurrenceNature').value=''; $v('v7OccurrenceDate').value=`${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}`;
  $v('v7OccurrenceTime').value=`${pad(now.getHours())}:${pad(now.getMinutes())}`; $v('v7OccurrenceBopm').value=''; $v('v7OccurrenceObs').value=''; $v('v7OccurrenceMsg').innerHTML='';
  $v('v7OccurrenceModal').classList.add('open');
}
window.openOccurrenceForBase=(idx)=>{const p=allVisitProps().find(x=>x.key==='base_'+idx); if(p)openOccurrenceForProperty(p);};
window.openOccurrenceForCloud=(id)=>{const p=allVisitProps().find(x=>x.key==='cloud_'+id); if(p)openOccurrenceForProperty(p);};
window.saveRuralOccurrence=async()=>{
  const btn=$v('btnSalvarOcorrencia'), msg=$v('v7OccurrenceMsg'); if(btn?.disabled)return;
  try{
    const p=allVisitProps().find(x=>x.key===$v('v7OccurrenceKey').value); if(!p)throw Error('Propriedade não localizada.');
    const natureza=$v('v7OccurrenceNature').value, dataLocal=$v('v7OccurrenceDate').value; if(!natureza)throw Error('Selecione a natureza.'); if(!dataLocal)throw Error('Informe a data.');
    btn.disabled=true; btn.innerHTML='⏳ Salvando...';
    await addDoc(collection(db,'ocorrencias_rurais'),{propriedadeId:p.id,propriedade:p.nome,origem:p.source,quadrante:p.quadrante||'',municipio:p.municipio||'Casa Branca',lat:p.lat,lng:p.lng,natureza,dataLocal,horaLocal:$v('v7OccurrenceTime').value||'',bopm:$v('v7OccurrenceBopm').value.trim(),observacao:$v('v7OccurrenceObs').value.trim(),usuario:v7User.email,usuarioNome:v7Profile?.nome||v7User.email,createdAt:serverTimestamp(),createdAtLocal:new Date().toISOString(),finalidade:'Inteligência territorial; não substitui BOPM'});
    await auditV7('ocorrencia_inteligencia_registrada',`${p.nome} · ${natureza}`); msg.innerHTML='<span style="color:#15803d">✅ Registro incluído na inteligência territorial.</span>';
    setTimeout(()=>closeV7Modal('v7OccurrenceModal'),1000);
...
function v13IntelligenceData(){
  ...
  const props=st.props||[], visits=st.visits||[], occ=(v7Occurrences||[]).map(o=>({...o,_dt:v13OccurrenceDate(o),_municipio:o.municipio||'Casa Branca'})).filter(o=>!scopeMun||o._municipio===scopeMun);
  const propKey=p=>normTxt(p.municipio||'Casa Branca')+'|'+normTxt(p.nome);
  const qData=[1,2,3,4].map(q=>{
    ...
    const never=qp.filter(p=>!st.latestByProp.has(propKey(p))).length;
    const overdue=qp.filter(p=>{const v=st.latestByProp.get(propKey(p));return !v||v13DaysSince(v._dt,now)>30}).length;
    ...
    const occurrenceRisk=recentOccurrences.length?Math.min(100,Math.round(recentOccurrences.slice().sort((a,b)=>b._dt-a._dt).slice(0,5).reduce((sum,o)=>{const days=Math.floor(Math.max(0,now-o._dt)/86400000),recencyFactor=days<=30?1:(days<=60?.75:.5);return sum+(occurrenceWeights[normTxt(o.natureza||'Outros').toLowerCase()]||50)*recencyFactor;},0)/Math.max(1,Math.min(recentOccurrences.length,3)))):0;
...
window.v14SaveDecision=async(status)=>{
  ...
  try{ await addDoc(collection(db,'planos_inteligencia'),{status,municipio:v15ReportScopeMunicipio()||'Toda a 2ª Companhia',quadrante:top.q,quadranteNome:v7QLabel(top.q),ippr:top.score,nivel:top.label,plano:w,observacaoComando:obs,usuario:v7User?.email||'',usuarioNome:v7Profile?.nome||v7User?.email||'',createdAt:serverTimestamp(),createdAtLocal:new Date().toISOString()}); await auditV7('decisao_inteligencia',`${status} · ${v7QLabel(top.q)} · IPPR ${top.score}`); const m=$v('v14DecisionMsg');if(m)m.innerHTML='<b style="color:#15803d">✅ Decisão registrada no histórico da inteligência.</b>'; }catch(e){alert('Não foi possível registrar a decisão: '+e.message);}
};
function v15ActionPlan(q,d){
  const occ=(q?.recentOccurrences||[]).slice().sort((a,b)=>b._dt-a._dt);
  const latest=occ[0]||null;
  const overdueProps=(q?.qp||[]).filter(p=>{
    const key=normTxt(p.municipio||'Casa Branca')+'|'+normTxt(p.nome);
    const v=d.st.latestByProp.get(key);
    return !v||v13DaysSince(v._dt,d.now)>30;
  }).slice(0,8);
  const occurrenceProps=[...new Map(occ.map(o=>[normTxt(o.propriedade||o.propriedadeId),o])).values()].slice(0,5);
  const targets=[...new Map([...occurrenceProps.map(o=>[normTxt(o.propriedade||o.propriedadeId),{nome:o.propriedade||'Propriedade da ocorrência',occ:o}]),...overdueProps.map(p=>[normTxt(p.nome),{nome:p.nome,occ:null}])]).values()].slice(0,8);
  const reasons=[];
  if(occ.length){
    const nature=[...new Set(occ.slice(0,5).map(o=>String(o.natureza||'Outros')))].join(', ');
    reasons.push(`${occ.length} ocorrência(s) territorial(is) nos últimos 90 dias (${nature})`);
  }
  if(q?.coverage30<70) reasons.push(`cobertura de ${q.coverage30}% nos últimos 30 dias`);
  if(q?.overdue) reasons.push(`${q.overdue} propriedade(s) sem visita há mais de 30 dias`);
  if(q?.harvest) reasons.push(`${q.harvest} propriedade(s) em período de colheita`);
  if(!reasons.length) reasons.push('indicadores territoriais sem fator crítico identificado');
  const what=latest
    ? `Priorizar presença ostensiva e visitas preventivas no ${v7QLabel(q.q)}, começando pelos locais vinculados às ocorrências recentes.`
    : `Intensificar visitas preventivas no ${v7QLabel(q.q)}, priorizando propriedades com maior tempo sem visita.`;
  const where=targets.length?targets.map(x=>x.nome).join(' · '):v7QLabel(q.q);
  const when=latest && v13DaysSince(latest._dt,d.now)<=30
    ? 'Prioridade imediata nos próximos 7 dias; considerar o horário do fato como referência para o planejamento.'
    : 'Próximos 7 dias, conforme disponibilidade operacional e demais demandas do serviço.';
  const how=latest
    ? 'Roteiro direcionado: verificar primeiro os pontos das ocorrências, realizar presença ostensiva nas proximidades e completar a cobertura nas propriedades sem visita recente.'
    : 'Roteiro direcionado às propriedades com maior intervalo sem visita, complementado por pontos de observação e contato preventivo.';
  const htmlTargets=targets.map(x=>{
    const o=x.occ;
    const extra=o?`<span class="v15-attention-reason">🚨 ${String(o.natureza||'Outros')} · ${o._dt?o._dt.toLocaleDateString('pt-BR'):''}</span>`:'<span class="v15-attention-reason">⏱️ Sem visita há mais de 30 dias</span>';
    const btn=o?`<button onclick="v14FocusOccurrence('${String(o.id||'').replace(/'/g,"\\'")}')" class="v15-mini-btn">📍 Mapa</button>`:'';
    return `<div class="v15-attention-row"><div><b>${String(x.nome||'Propriedade').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}</b><div class="v7-small">${v7QLabel(q.q)} · ${extra}</div></div>${btn}</div>`;
  }).join('')||'<div class="v7-small">Nenhuma propriedade prioritária identificada neste quadrante.</div>';
  return {what,why:reasons.join('; '),where,when,how,targets:htmlTargets,meta:`${targets.length} alvo(s) sugerido(s)`};
}
function v15AttentionAreas(d){
  return d.propRows.filter(x=>x.occurrences>0 || x.days>30 || x.days===9999).sort((a,b)=>b.score-a.score).slice(0,8);
}
window.openRuralIntelligence=()=>{
  ...
  if((window.V15_ACTIVE_AREA||'Casa Branca')==='2cia') return alert('Para o IPPR por quadrante, selecione primeiro um município. A visão consolidada da 2ª Companhia será a próxima camada gerencial.');
  const d=v13IntelligenceData(),top=d.qData[0],w=v13FiveW2H(top,d),action=v15ActionPlan(top,d),attention=v15AttentionAreas(d),el=$v('v7IntelContent');
  const qcards=d.qData.map(x=>{const lastOcc=x.recentOccurrences.slice().sort((a,b)=>b._dt-a._dt)[0]; const occText=lastOcc?` · Última: ${lastOcc.natureza||'Outros'} — ${lastOcc.propriedade||'propriedade não informada'} (${lastOcc._dt.toLocaleDateString('pt-BR')})`:''; return `<div class="v7-card" style="border-left:5px solid ${v13PriorityColor(x.score)}"><div style="display:flex;justify-content:space-between;gap:8px"><b>${v7QLabel(x.q)}</b><b style="color:${v13PriorityColor(x.score)}">IPPR ${x.score} · ${x.label}</b></div><div class="v7-small">Cobertura 30d: ${x.coverage30}% · +30d sem visita: ${x.overdue}/${x.qp.length} · Plantio: ${x.planting} · Colheita: ${x.harvest} · Ocorrências 90d: ${x.recentOccurrences.length}${occText}</div></div>`;}).join('');
  const topQuadrantRows=d.propRows.filter(x=>String(x.p.quadrante)===String(top.q)).slice(0,10);
  const otherAttentionRows=d.propRows.filter(x=>String(x.p.quadrante)!==String(top.q)).slice(0,10);
  const occurrenceCell=x=>x.latestOccurrence?`${x.latestOccurrence.natureza||'Outros'} · ${x.latestOccurrence._dt.toLocaleDateString('pt-BR')}`:'—';
  const priority=topQuadrantRows.map((x,i)=>`<tr><td>${i+1}</td><td>${x.p.nome}</td><td>${v7QLabel(x.p.quadrante)}</td><td>${x.days===9999?'Nunca':x.days+' dias'}</td><td>${x.harvest?'Colheita ':''}${x.planting?'Plantio':''}</td><td>${occurrenceCell(x)}</td><td>${x.occurrences}</td><td><b>${x.score}</b></td></tr>`).join('');
  const otherPriority=otherAttentionRows.map((x,i)=>`<tr><td>${i+1}</td><td>${x.p.nome}</td><td>${v7QLabel(x.p.quadrante)}</td><td>${x.days===9999?'Nunca':x.days+' dias'}</td><td>${x.harvest?'Colheita ':''}${x.planting?'Plantio':''}</td><td>${occurrenceCell(x)}</td><td>${x.occurrences}</td><td><b>${x.score}</b></td></tr>`).join('');
  el.innerHTML=`<div class="v14-intel"><div class="v7-card v14-explain"><b>🧠 IPPR V2 — Índice de Prioridade de Policiamento Rural · ${window.V15_ACTIVE_AREA||'Casa Branca'}</b><div class="v7-small">Ocorrências territoriais 90d (40%) + baixa cobertura 30d (25%) + atraso de visitas (20%) + colheita (10%) + situações de atenção em campo (5%). Plantio não aumenta a prioridade do IPPR. Ocorrências mais recentes recebem maior peso dentro dos últimos 90 dias. É apoio à decisão; não determina emprego automático de efetivo.</div></div><h3>🚨 Painel de ocorrências rurais</h3>${v14OccurrencePanel(d)}<h3>Prioridade por quadrante</h3>${qcards}<h3>🎯 Plano de ação sugerido · ${v7QLabel(top.q)}</h3><div class="v7-card v15-action-card"><div class="v15-action-title"><b>${action.what}</b><span>${action.meta}</span></div><div class="v15-action-grid"><div><b>POR QUÊ?</b><br>${action.why}</div><div><b>ONDE?</b><br>${action.where}</div><div><b>QUANDO?</b><br>${action.when}</div><div><b>COMO?</b><br>${action.how}</div></div></div><div class="v7-card v15-attention-card"><div style="display:flex;justify-content:space-between;gap:8px;align-items:center"><b>🚨 Áreas que exigem atenção</b><span class="v7-small">prioridade automática · ${attention.length} alvos</span></div><div class="v7-small" style="margin:4px 0 8px">O sistema cruza ocorrência recente, tempo sem visita e prioridade da propriedade. A lista serve para orientar o roteiro, não para determinar emprego automático de efetivo.</div>${attention.map(x=>{const o=x.latestOccurrence;const reason=o?`🚨 ${o.natureza||'Outros'} · ${o._dt?o._dt.toLocaleDateString('pt-BR'):''}`:(x.days===9999?'⏱️ Nunca visitada':`⏱️ ${x.days} dias sem visita`);const btn=o?`<button onclick="v14FocusOccurrence('${String(o.id||'').replace(/'/g,"\'")}')" class="v15-mini-btn">📍 Mapa</button>`:'';return `<div class="v15-attention-row"><div><b>${String(x.p.nome||'Propriedade').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}</b><div class="v7-small">${v7QLabel(x.p.quadrante)} · ${reason} · prioridade ${x.score}</div></div>${btn}</div>`;}).join('')||'<div class="v7-small">Nenhuma área prioritária identificada.</div>'}</div><h3>5W2H gerencial · ${v7QLabel(top.q)}</h3><div class="v7-card v14-plan"><b>WHAT — O quê?</b><br>${w.what}<hr><b>WHY — Por quê?</b><br>${w.why}<hr><b>WHERE — Onde?</b><br>${w.where}<hr><b>WHEN — Quando?</b><br>${w.when}<hr><b>WHO — Quem?</b><br>${w.who}<hr><b>HOW — Como?</b><br>${w.how}<hr><b>HOW MUCH — Quanto?</b><br>${w.howmuch}</div><div class="v7-card"><b>Decisão do comando</b><div class="v7-small">Registre a decisão gerencial sobre a recomendação. Isso não altera automaticamente escalas ou emprego de efetivo.</div><textarea id="v14DecisionObs" class="field-inp" style="min-height:65px" placeholder="Observação do comando (opcional)"></textarea><div class="v14-actions"><button onclick="v14SaveDecision('APROVADO')">✅ Aprovar</button><button onclick="v14SaveDecision('AJUSTAR')">✏️ Ajustar</button><button onclick="v14SaveDecision('DESCONSIDERADO')">➖ Desconsiderar</button></div><div id="v14DecisionMsg" class="v7-small"></div></div><h3>Propriedades prioritárias · ${v7QLabel(top.q)}</h3><div class="v7-small v141-list-note">Primeiro são apresentadas as propriedades do quadrante apontado pelo IPPR como prioridade operacional.</div><div style="overflow:auto"><table class="v14-table"><tr><th>#</th><th>Propriedade</th><th>Quadrante</th><th>Última visita</th><th>Sazonalidade</th><th>Ocorrência recente</th><th>Ocorr. 90d</th><th>Prioridade</th></tr>${priority||'<tr><td colspan="8">Nenhuma propriedade disponível neste quadrante.</td></tr>'}</table></div><h3>Outras propriedades que exigem atenção</h3><div class="v7-small v141-list-note">Ranking complementar dos demais quadrantes, sem substituir a prioridade territorial definida acima.</div><div style="overflow:auto"><table class="v14-table"><tr><th>#</th><th>Propriedade</th><th>Quadrante</th><th>Última visita</th><th>Sazonalidade</th><th>Ocorrência recente</th><th>Ocorr. 90d</th><th>Prioridade</th></tr>${otherPriority||'<tr><td colspan="8">Nenhuma outra propriedade exige atenção no momento.</td></tr>'}</table></div><div class="v7-card"><b>Ciclo de gestão</b><div class="v7-small">Diagnosticar → Planejar (5W2H) → Decidir → Executar → Medir cobertura/visitas → Reavaliar IPPR. A decisão operacional permanece com o comando.</div></div></div>`;
  $v('v7IntelModal').classList.add('open');
};

window.openCommanderReport=()=>{
  // V15.4.2: abre a nova aba imediatamente no clique para evitar bloqueio de popup
  // e isola falhas de montagem do relatório com mensagem visível ao usuário.
  const reportWindow=window.open('','_blank');
  if(!reportWindow){ alert('O navegador bloqueou a abertura do relatório. Libere pop-ups para o SISRURAL e tente novamente.'); return; }
  try{
    reportWindow.document.open();
    reportWindow.document.write('<!doctype html><html><head><meta charset=\"utf-8\"><title>Gerando relatório...</title></head><body style=\"font-family:Arial;padding:24px\">Gerando relatório operacional...</body></html>');
  ...
  reportWindow.focus();
    ...
    console.error('SISRURAL relatório operacional:',e);
    try{ reportWindow.document.open(); reportWindow.document.write('<!doctype html><html><head><meta charset=\"utf-8\"><title>Erro no relatório</title></head><body style=\"font-family:Arial;padding:24px\"><h2>Não foi possível gerar o relatório</h2><p>'+String(e?.message||e)+'</p><button onclick=\"window.close()\">Fechar</button></body></html>'); reportWindow.document.close(); }catch(_e){}
    alert('Não foi possível gerar o relatório operacional: '+(e?.message||e));
  }
};
